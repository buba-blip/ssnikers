...
function updateCartDisplay() {
  ...
  cart.forEach((item) => {
    ...
    cartItem.innerHTML = `<div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #eee;">
                <div>
                ...
                <div>
                    ...
                    <button onclick="removeFromCart('${
                      item.product
                    }')" style="margin-left: 10px; background: #ff4757; color: white; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer;">×</button>
                </div>
            </div>`;
...
async function loadProducts() {
  try {
    ...
    grid.innerHTML = ""; // очищаємо попередні товари
      ...
      card.innerHTML = `
        <div class="product-image">
          ...
          <div class="product-overlay">
            <img src="${product.overlay}" alt="${product.name}">
          </div>
        </div>
    ...
    // після рендеру — знову активуємо анімації
    observeProductCards();
  } catch (err) {
